...
struct Washroom* createWashroom() {
    ...
    washroom->man_num = 0;
    washroom->wom_num = 0;
    washroom->not_full  = uthread_sem_create (0);
    return washroom;
};
...
void leaveWashroom() {
    for (int i = 0; i < NUM_PEOPLE; i++) {
        ...
        uthread_sem_signal(washroom->not_full);
    }
...
void* male(int number) {
    ...
    for (int i = 0; i < NUM_ITERATIONS; i++) {
        ...
        while (washroom->wom_num > 0 || washroom->man_num == MAX_OCCUPANCY) {
            waiting++;
            uthread_sem_wait(washroom->not_full);
...
int main (int argc, char** argv) {
    ...
    waitingHistogrammutex = uthread_sem_create (0);
    washroom = createWashroom();
